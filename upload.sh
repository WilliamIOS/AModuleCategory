...
DevelopVersionString=`grep -E 'version.develop.*=' AModuleCategory.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' AModuleCategory.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' AModuleCategory.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' AModuleCategory.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" AModuleCategory.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' AModuleCategory.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" AModuleCategory.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' AModuleCategory.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" AModuleCategory.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' AModuleCategory.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' AModuleCategory.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" AModuleCategory.podspec
...
pod repo push ${Repo} AModuleCategory.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
